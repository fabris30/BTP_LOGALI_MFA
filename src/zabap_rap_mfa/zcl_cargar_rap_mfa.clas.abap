CLASS zcl_cargar_rap_mfa DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_cargar_rap_mfa IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.

    out->write( 'Adding Travel data' ).

    DELETE FROM z360_travel_mfa.
    DELETE FROM z360_travel_mfd.

    INSERT z360_travel_mfa FROM (
    ...
    DELETE FROM z360_booking_mfa.
    DELETE FROM z360_booking_mfd.

    INSERT z360_booking_mfa FROM (

        SELECT
          FROM /dmo/booking
          JOIN z360_travel_mfa ON /dmo/booking~travel_id = z360_travel_mfa~travel_id
          JOIN /dmo/travel ON /dmo/travel~travel_id = /dmo/booking~travel_id
          FIELDS  "client,
                  uuid( ) AS booking_uuid,
                  z360_travel_mfa~travel_uuid AS parent_uuid,
                  ...
                  z360_travel_mfa~last_changed_at AS local_last_changed_at ).


    DELETE FROM z360_bksuppl_mfa.
    DELETE FROM z360_bksuppl_mfd.

    out->write( 'Adding Booking Supplements data' ).

    INSERT z360_bksuppl_mfa FROM (
       SELECT FROM /dmo/book_suppl AS supp
              JOIN z360_travel_mfa  AS trvl ON trvl~travel_id = supp~travel_id
              JOIN z360_booking_mfa AS book ON book~parent_uuid = trvl~travel_uuid
